...
class CarDataVisualizer:
    ...
    def _initialize_plot(self):
        self.fig, self.axes = plt.subplots(1,1,figsize=(12,6))
        ...
        self.axes.set_title(f"Car 1 Data -> Coverage 0%")
        self.axes.set_xlabel("Distance (m)")
        self.axes.set_ylabel("Relative Velocity (m/s)")
    ...
    def update(self, frame):
        ...
        self.axes.scatter(lon, speed, c='red')
        self.axes.set_title(f"Car 1 Data -> Coverage {len(covered_grid)/self.grid_size**2 * 100 :.2f}%")
        self.axes.set_xlabel("Distance (m)")
        self.axes.set_ylabel("Relative Velocity (m/s)")
    ...
    def run(self):
        ...
        ani = FuncAnimation(self.fig, self.update, interval=self.update_interval, save_count=50)
        plt.show()

if __name__ == "__main__":
    visualizer = CarDataVisualizer(update_interval=100)
    visualizer.run()